...
Верни 10 горячих клавиш (сочетания могут использовать Ctrl, Cmd, Alt, Shift или быть отдельными кнопками вроде F2/F4) для программы "${cleanTopic}".

СТРОГИЕ ПРАВИЛА (нарушение недопустимо):
1. НЕ ПРИДУМЫВАЙ комбинации. Используй только те горячие клавиши, которые реально существуют и задокументированы в официальной справке/документации программы "${cleanTopic}".
2. Никакой отсебятины в описаниях: поле "desc" должно точно и нейтрально описывать действие на ${AI_LANG_HINT[lang]}.
3. Поле "key" — ТОЛЬКО ОДНА строчная английская буква, цифра или название клавиши (например "f4", "=", "`").
4. Не повторяй одну и ту же комбинацию дважды.
5. Верни ТОЛЬКО чистый валидный JSON-массив объектов. Без markdown, без пояснений, без текста до или после массива.

Формат строго такой:
[
  {"desc": "Описание действия", "key": "c", "shift": false, "ctrl": true, "alt": false, "visual": "Ctrl + C"},
  {"desc": "Автосумма", "key": "=", "shift": false, "ctrl": false, "alt": true, "visual": "Alt + ="}
]`;
