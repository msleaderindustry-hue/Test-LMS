...
const DEFAULT_LEVELS = {
    ...
    css: {
        ...
        hiddenCss: "body { display:flex; justify-content:center; align-items:center; height:100vh; background:#0f172a; margin:0; } .target-element { padding:15px 35px; font-size:18px; font-weight:bold; border:2px solid #10b981; border-radius:8px; cursor:pointer; transition:0.3s; }",
        palette: [
    ...
    js: {
        ...
        hiddenCss: "body { display:flex; justify-content:center; align-items:center; height:100vh; background:#0f172a; transition:background 0.5s ease; margin:0; }",
        palette: [
...
const WebBuilderLMS = ({ onBack }) => {
    ...
    useEffect(() => {
        ...
        if (mode === 'html' || mode === 'js') {
            // Для HTML и JS порядок КРИТИЧЕСКИ важен
            isWin = (currentIds === expectedIds);
        } else if (mode === 'css') {
            // Для CSS порядок свойств не важен
    ...
    const generateAILevel = async () => {
        ...
        if (mode === 'html') {
            ...
            {"title":"Название","task":"Описание задачи","baseHtml":"<div class='target-element'>Текст</div>","hiddenCss":"body{background:#0f172a; display:flex; justify-content:center; align-items:center; height:100vh;}","expected":["id1","id2"],"palette":[{"id":"id1","label":"color: white;","desc":"ЦВЕТ ТЕКСТА","code":"color: white;","color":"#8b5cf6"}]}
    ...
    const renderPreview = () => {
        ...
        return (
            <iframe srcDoc={srcDoc} sandbox="allow-scripts allow-modals" style={{ width: '100%', height: '100%', border: 'none', background: '#0f172a' }} />
        );
    };
        ...
        <motion.div className="glass-panel" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} style={{ width: '100%', maxWidth: '1200px', padding: '20px', margin: '0 auto', borderRadius: '16px', background: '#1e293b' }}>
            ...
            <header style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h2 style={{ margin: 0, fontSize: '26px', fontWeight: 900, color: '#f8fafc' }}>{level?.title}</h2>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#94a3b8', fontSize: '14px', marginTop: '5px' }}>
                    <span style={{color: '#ef4444', fontSize: '18px'}}>🎯</span> <b style={{color: '#e2e8f0'}}>Задача:</b> {level?.task}
                </div>
            </header>

            <div style={{display: 'flex', gap: '10px', marginBottom: '20px'}}>
                <Button variant="muted" onClick={()=>setMode('html')} style={{border: mode === 'html' ? '1px solid #0ea5e9' : '1px solid #334155', color: mode==='html'?'#0ea5e9':'#94a3b8'}}>🏗️ Структура (HTML)</Button>
                <Button variant="muted" onClick={()=>setMode('css')} style={{border: mode === 'css' ? '1px solid #8b5cf6' : '1px solid #334155', color: mode==='css'?'#8b5cf6':'#94a3b8'}}>🎨 Дизайн (CSS)</Button>
                <Button variant="muted" onClick={()=>setMode('js')} style={{border: mode === 'js' ? '1px solid #f59e0b' : '1px solid #334155', color: mode==='js'?'#f59e0b':'#94a3b8'}}>⚡ Интерактив (JS)</Button>
            </div>

            <div style={{ display: 'flex', gap: '10px', background: '#0f172a', border: '1px solid #334155', padding: '15px', borderRadius: '12px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{fontSize: '20px'}}>🧠</span>
                <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Попроси ИИ создать новую задачу (напр: Кнопка лайка, Модальное окно)" style={{ flex: 1, minWidth: '200px', padding: '10px 15px', borderRadius: '8px', border: '1px solid #334155', background: '#1e293b', color: '#f8fafc', outline: 'none' }} disabled={isGenerating} />
            ...
            <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'stretch' }}>
                ...
                <div style={{ flex: '1 1 250px', background: '#0f172a', borderRadius: '12px', border: '1px solid #1e293b', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '15px', fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', borderBottom: '1px solid #1e293b' }}>Доступные фрагменты кода</div>
                    <div className="modern-scroll" style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto', maxHeight: '450px' }}>
                        {(level?.palette || []).map((block) => (
                            <motion.button 
                                ...
                                style={{ width: '100%', textAlign: 'left', padding: '12px 15px', background: '#1e293b', border: `1px solid ${block.color}`, borderRadius: '8px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '4px' }}
                            >
                ...
                {showWinModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'absolute', inset: 0, background: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(5px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '16px' }}>
                        <motion.div initial={{ scale: 0.8, y: 50 }} animate={{ scale: 1, y: 0 }} style={{ background: '#1e293b', padding: '40px', borderRadius: '24px', maxWidth: '600px', width: '100%', textAlign: 'center', border: '1px solid #334155', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
                            ...
                            <p style={{ color: '#cbd5e1', fontSize: '16px', marginBottom: '20px' }}>Ты собрал рабочий код визуально. А вот так это выглядит <b>в настоящем редакторе</b>:</p>
