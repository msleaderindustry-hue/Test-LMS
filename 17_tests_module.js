...
(function () {
    ...
    const TestsLMS = ({ view, setView, currentSet, tests, setTests, user, history, setHistory, fp, sets, addSet, deleteSet, openSet, teacherTests, openTeacherAssignedTest, removeTeacherTestStudent }) => {
        ...
        // --- РЕНДЕР MATHJAX ПРИ СМЕНЕ ВОПРОСА ---
        useEffect(() => {
        ...
        return (
            <AnimatePresence mode="wait">
                ...
                {/* --- ОБНОВЛЕННЫЙ ИНТЕРФЕЙС ТЕСТА СОГЛАСНО ДИЗАЙНУ МАМКЕТА --- */}
                {view === 'test' && (
                    <motion.div key="test-wrapper" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{ display: 'flex', gap: '30px', width: '100%', maxWidth: '1100px', margin: '0 auto', alignItems: 'flex-start' }}>
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <motion.div 
                                            ...
                                            style={{ background: '#ffffff', borderRadius: '24px', padding: '40px', boxShadow: '0 4px 30px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9' }}
                                        >
                                            ...
                                            {/* Текст вопроса */}
                                            <div style={{ fontSize: '24px', fontWeight: 500, color: '#1e293b', marginBottom: '30px', lineHeight: '1.4' }} dangerouslySetInnerHTML={{ __html: q.question }} />

                                            {/* Изображение вопроса (сохранение старого формата рендера) */}
                                            {q.questionImg && (
                                            ...
                                            {/* Варианты ответов (Настоящие карточки с радио-кнопками) */}
                                                    ...
                                                    return (
                                                        <div key={idx} onClick={() => handleAnswer(idx)}
                                                            ...
                                                            <div style={{
                                                                ...
                                                                background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease'
                                                            }}>
                                                                {isSelected && <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: '#4f46e5' }} />}
                                                            </div>
                                                            
                                                            {/* Текст варианта */}
                                                            <div style={{ fontSize: '16px', color: isSelected ? '#1e293b' : '#475569', fontWeight: isSelected ? 600 : 400 }}
                        ...
                        <div style={{ width: '340px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '24px' }}>
                            ...
                            {(() => {
                                ...
                                const timerTextColor = isLowTime ? '#ef4444' : '#1e293b';
                                const timerCardBg = isLowTime ? '#fef2f2' : '#ffffff';
                                const timerBorder = isLowTime ? '1px solid #fecaca' : '1px solid #f1f5f9';
                                ...
                                return (
                                    <div style={{ background: timerCardBg, borderRadius: '24px', padding: '25px', boxShadow: '0 4px 30px rgba(0,0,0,0.04)', border: timerBorder, display: 'flex', alignItems: 'center', gap: '16px', transition: 'all 0.3s ease' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ fontSize: '13px', color: '#64748b', marginTop: '6px' }}>осталось времени</div>
                                        </div>
                                    </div>
                                );
                            })()}
                            ...
                            {/* Сетка вопросов */}
                            <div style={{ background: '#ffffff', borderRadius: '24px', padding: '25px', boxShadow: '0 4px 30px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9' }}>
                                <div style={{ fontSize: '14px', color: '#64748b', marginBottom: '20px', fontWeight: 500 }}>
                                    Отвечено: <span style={{ color: '#1e293b', fontWeight: 700 }}>{testSession.answers.filter(a => a !== null).length}</span> из {testSession.questions.length}
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '10px' }}>
                                    {testSession.questions.map((_, i) => {
                                        let bg = '#f8fafc'; // неотвеченные — нейтральные
                                        let color = '#475569';
                                        let border = '1px solid #e2e8f0';

                                        if (i === testSession.currentIdx) {
                                            bg = '#8b5cf6'; // текущий вопрос — фиолетово-пурпурная заливка
                                            color = '#ffffff';
                                            border = '1px solid #8b5cf6';
                                        } else if (testSession.answers[i] !== null) {
                                            bg = 'linear-gradient(135deg, #f3e8ff, #e0e7ff)'; // отвеченные — мягкая подсветка того же градиента
                                            color = '#7c3aed';
                            ...
                            {/* Кнопка Завершить - фирменный фиолетово-голубой градиент */}
