...
const Input = (props) => (
  <input {...props} style={{ width: '100%', padding: '0 18px', borderRadius: '14px', border: '2px solid transparent', background: 'var(--input-bg)', color: 'var(--text-main)', fontSize: '16px', fontWeight: 500, marginBottom: '12px', marginTop: '8px', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.03)', transition: 'all 0.3s', ...props.style }} />
);
     ...
     return (
       <motion.div ref={cardRef} key={index} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="glass-panel" style={{width: '100%', display:'block'}}>
          ...
          {question.questionImg && (
              <img src={question.questionImg} className="question-image" />
          )}
...
const StatsView = ({ history, setHistory, onBack }) => {
    ...
    useEffect(() => {
        ...
        const c = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ label: '%', data: sorted.slice(0,10).map(i=>i.percent), backgroundColor:'#667eea', borderRadius:4 }]
            },
            options: {
                scales: { y: { beginAtZero: true, max: 100, ticks:{color:'#718096'} }, x:{ ticks:{color:'#718096'} } },
    ...
    return (
        <motion.div key="stats" initial={{opacity:0}} animate={{opacity:1}} className="glass-panel" style={{width:'100%', maxWidth:800, maxHeight:'90vh', overflowY:'auto', display:'block'}}>
            ...
            <div style={{background:'var(--variant-default)', padding:15, borderRadius:20, marginBottom:25, height:220}}>
               <canvas ref={chartRef}></canvas>
            </div>

            <table style={{width:'100%', borderCollapse:'collapse'}}>
                <thead>
                    <tr style={{borderBottom:'2px solid rgba(0,0,0,0.1)', color:'var(--text-sec)'}}>
                        <th style={{textAlign:'left', padding:10}}>Студент</th>
                    ...
                    {sorted.map(h => (
                        <tr key={h.id} style={{borderBottom:'1px solid rgba(128,128,128,0.1)'}}>
                            ...
                            <td style={{textAlign:'right'}}>
                                ...
                                }} style={{border:'none', background:'transparent', color:'var(--text-sec)', fontSize:18}}>✕</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
...
function App() {
    ...
    async function check() {
      ...
      if(saved) {
        try {
          ...
          if(obj.fingerprint === f) {
            const expected = await hmacSign(SECRET_KEY, f);
            if(obj.license === expected) valid = true;
          }
        } catch(e){}
      }
      
      if(valid) { loadData(); setView('menu'); } 
  ...
  const loadData = () => {
    ...
    setSets(raw ? JSON.parse(raw) : ['Электроника']);
    if(!raw) {
        localStorage.setItem('test_sets_list', JSON.stringify(['Электроника']));
        localStorage.setItem('tests_Электроника', JSON.stringify([]));
    }
  ...
  const launchTestWithTimer = () => {
      ...
      setTimeLeft(mins * 60);
      setTestSession({
          questions: finalQuestions, currentIdx: 0,
          answers: new Array(finalQuestions.length).fill(null), score: 0
      });
      setView('test');
  };
  ...
  const finishTest = () => {
      let correct = 0;
      testSession.questions.forEach((q, i) => { if(testSession.answers[i] === q.correctIndex) correct++; });
  ...
  const restartMistakes = () => {
      ...
      const reshuffledQuestions = wrongQuestions.map(q => {
          ...
          return { ...q, variants: newVariants, correctIndex: newCorrectIndex };
      });

      const mins = parseInt(customTime) || 20;
      setTimeLeft(mins * 60);
      setTestSession({
          questions: reshuffledQuestions, currentIdx: 0,
          answers: new Array(reshuffledQuestions.length).fill(null), score: 0
      });
      setIsResultSaved(false);
      setView('test');
  };
