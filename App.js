...
function App() {
  ...
  // --- НОВАЯ ФУНКЦИЯ ДЛЯ ПОВТОРЕНИЯ ОШИБОК ---
  const restartMistakes = () => {
    // Фильтруем вопросы, выбираем только те, где ответ не совпадает с правильным индексом
    const wrongQuestions = testSession.questions.filter((q, i) => testSession.answers[i] !== q.correctIndex);
    
    if(wrongQuestions.length === 0) return; // На всякий случай

    // Сбрасываем таймер на исходное время
    const mins = parseInt(customTime) || 20;
    setTimeLeft(mins * 60);
