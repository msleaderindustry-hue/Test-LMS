 ...
 * ExcelTrainerLMS
 * ---------------------------------------------------------------------------
 * Код UI написан начисто с инкапсуляцией стилей и поддержкой тем.
 * Логика работы (AI, проверка, переводы, Firebase) полностью сохранена.
 * Интерфейс разбит на модульные компоненты для удобства поддержки.
 * ---------------------------------------------------------------------------
 */
(function () {
    ...
    const PALETTES = {
        dark:  { bg:'var(--bg-body, #050816)', panel:'var(--bg-panel, #0D1328)', card:'var(--bg-card, #111936)', text:'var(--text-main, #F8FAFC)', sub:'var(--text-sec, #94A3B8)', border:'var(--glass-border, rgba(255,255,255,0.08))', tableBg:'#0f172a', tableText:'#e2e8f0', tableBorder:'#1e293b', tableHead:'#111936' },
        light: { bg:'var(--bg-body, #f1f5f9)', panel:'var(--bg-panel, #ffffff)', card:'var(--bg-card, #f8fafc)', text:'var(--text-main, #0f172a)', sub:'var(--text-sec, #64748b)', border:'var(--glass-border, #e2e8f0)', tableBg:'#ffffff', tableText:'#1e293b', tableBorder:'#e2e8f0', tableHead:'#f8fafc' }
    };
    ...
    /* =========================================================================
       ...
       ========================================================================= */
    const XM_STYLES = `
    .xm-shell { --xm-radius-sm:12px; --xm-radius-md:18px; --xm-radius-lg:24px; width:100%; max-width:1240px; margin:0 auto; background: var(--xm-bg); border-radius: var(--xm-radius-lg); padding: 24px; box-sizing:border-box; font-family: 'Inter', sans-serif; color: var(--xm-text); transition: background 0.3s ease; }
    ...
    .xm-lang-switch { display:flex; gap:5px; background: rgba(0,0,0,.05); padding:5px; border-radius:14px; border:1px solid var(--xm-border); }
    ...
    .xm-theme-btn, .xm-menu-btn { width:38px; height:38px; border-radius:10px; border:1px solid var(--xm-border); background: var(--xm-card); color: var(--xm-text); cursor:pointer; font-size:15px; display:flex; align-items:center; justify-content:center; }
    ...
    .xm-category-header:hover { background: rgba(128,128,128,.05); }
    ...
    .xm-progress-bar { height:9px; border-radius:6px; background: rgba(128,128,128,.2); overflow:hidden; }
    ...
    .xm-skel-line { height:14px; border-radius:7px; background: linear-gradient(90deg, rgba(128,128,128,.1) 25%, rgba(128,128,128,.2) 37%, rgba(128,128,128,.1) 63%); background-size:400% 100%; animation: xm-shimmer 1.4s ease infinite; }
    ...
    .xm-syntax-code { font-size:14.5px; color:${ACCENTS.cyan}; font-family:'Fira Code', monospace; white-space:pre-wrap; line-height:1.6; display:block; padding-right:80px; }
    .xm-copy-btn { position:absolute; top:14px; right:14px; background: rgba(128,128,128,.1); border:1px solid rgba(128,128,128,.2); color:var(--xm-sub); padding:6px 11px; border-radius:8px; font-size:11px; font-weight:700; cursor:pointer; transition:.2s; }
    .xm-copy-btn:hover { color:var(--xm-text); border-color:${ACCENTS.cyan}; }
    ...
    .xm-toast { background: var(--xm-panel); color:var(--xm-text); padding:12px 18px; border-radius:12px; font-size:13px; font-weight:700; box-shadow:0 10px 30px rgba(0,0,0,.35); border:1px solid rgba(128,128,128,.2); display:flex; align-items:center; gap:8px; }
    ...
    const ExcelTrainerLMS = ({ onBack }) => {
        ...
        const generateAIFormula = async (formulaName, isRetry = false) => {
            ...
            Верни ТОЛЬКО чистый валидный JSON (без markdown) строго в таком формате:
            {
              ...
              "hint": {
                 "ru": ["Общая подсказка без готовой формулы", "Подсказка 2 (с чего начать формулу, например =СУММ()"],
            ...
            1. В "syntax" — ТОЛЬКО примеры формул с абстрактными ячейками.
            ...
            4. В "hint" НИКОГДА не пиши готовую формулу целиком.
            5. Экранируй внутренние кавычки в JSON.
            6. Тема задачи: "${randomTheme}". Поля "def", "table" и "taskDesc" должны быть ИМЕННО на эту тему, без шаблонных слов.
            7. "difficulty" должна реально отражать сложность функции, а "xp" соответствовать ей (easy=50, medium=100, hard=150).
            8. Перед выводом JSON мысленно проверь, что значения таблицы математически дают именно "result".`;
            ...
            try {
                ...
                if (!parsedFormula.hint) {
                    parsedFormula.hint = {
                        ru: [`Подумайте, какая функция нужна для "${formulaName}".`, `Формула начинается с =${formulaName}(`],
                        en: [`Think about which function you need.`, `The formula starts with =${parsedFormula.enName || formulaName}(`],
                        uz: [`Қайси функция кераклигини ўйланг.`, `Формула =${formulaName}( билан бошланади`]
                    };
                }
