...
.et-theme-toggle{width:38px;height:38px;border-radius:12px;border:1px solid var(--border);background:var(--bg-card);color:var(--text-main);
  cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;transition:.2s;}
.et-theme-toggle:hover{border-color:var(--accent-cyan);}
...
function ThemeToggle({ theme, setTheme }) {
    return (
        <button className="et-theme-toggle" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title="Theme">
            {theme === "dark" ? "☾" : "☀"}
        </button>
    );
}
