...
const ExcelTrainerLMS = ({ onBack }) => {
    ...
    const generateAIFormula = async (formulaName) => {
        ...
        // МАССИВ СЛУЧАЙНЫХ ТЕМ: ИИ будет вынужден каждый раз брать новую тему
        const themes = [
            "успеваемость и оценки студентов в университете",
            "статистика забитых голов в футбольном турнире",
            "инвентаризация запасов овощей в ресторане",
            "расчет сметы на строительство загородного дома",
            "учет проданных автомобилей в автосалоне",
            "дневник температур и прогноза погоды",
            "планирование семейного бюджета на море",
            "учет строительных инструментов на складе",
            "результаты школьной олимпиады по физике",
            "расходы на логистику и доставку грузов",
            "статистика кассовых сборов кинотеатра",
            "учет абонементов в фитнес-клубе",
            "учет медицинских препаратов в аптеке",
            "затраты на содержание домашних питомцев",
            "расписание и пассажиры авиарейсов"
        ];
        // Выбираем случайную тему из списка
        const randomTheme = themes[Math.floor(Math.random() * themes.length)];

        // ЖЕСТКИЙ ПРОМПТ ДЛЯ ИИ с динамической темой
        ...
        6. ТЕМА ЗАДАЧИ: СТРОГО используй тему "${randomTheme}". Сгенерируй таблицу, названия колонок и текст задачи ИМЕННО на эту тему! Не используй заезженные слова вроде "Иванов" или "Ноутбук".`;
