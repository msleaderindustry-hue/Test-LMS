...
const ExcelTrainerLMS = ({ onBack }) => {
    ...
    const generateAIFormula = async (formulaName) => {
        // Жестко сбрасываем поле ввода и зеленую плашку успеха при каждой новой генерации
        setInputValue("=");
        setShowSuccess(false);
        setIsGenerating(true);
        setCurrentLesson(null);

        // ФИКС ТУТ: Добавлено жесткое правило №2 с примерами ПЛОХО/ОТЛИЧНО для текста задачи
        ...
        Верни ТОЛЬКО чистый валидный JSON (без markdown) строго в таком формате:
        {
          ...
          "taskDesc": "Текст практической задачи (БЕЗ УКАЗАНИЯ ЯЧЕЕК И ДИАПАЗОНОВ).",
          "table": [
        ...
        1. СИНТАКСИС БЕЗ ТЕКСТОВЫХ ПОДСКАЗОК: В поле "syntax" КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать слова "диапазон", "критерий", "условие", "искомое_значение", "диапазон_суммирования". Нейросеть, это очень важно!
           ❌ ПЛОХО: =СУММЕСЛИ(диапазон; критерий; [диапазон_суммирования])
           ✅ ОТЛИЧНО: =СУММЕСЛИ(A2:A10; ">100"; B2:B10)
           Используй ТОЛЬКО адреса ячеек (A1, C5:C10) и конкретные значения (текст или числа) внутри скобок!
        2. ФОРМУЛИРОВКА ЗАДАЧИ БЕЗ ПОДСКАЗОК ("taskDesc"): КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ писать адреса ячеек в тексте задания! Не говори ученику, в какую ячейку писать результат (например, "В ячейке E2...") и не называй диапазоны (например, "Используйте диапазон A2:A7"). Ученик должен сам посмотреть на таблицу и понять, какие колонки использовать!
           ❌ ПЛОХО: В ячейке E2 напишите формулу для расчета общей суммы... Используйте диапазон A2:A7...
           ✅ ОТЛИЧНО: Напишите формулу, которая посчитает общую сумму оценок только по предмету "Математика".
        3. ЛОГИКА ОЖИДАЕМОГО ОТВЕТА ("expected"): В массив expected добавь ВСЕ возможные правильные варианты написания формулы, которая решает задачу. Обязательно убедись, что адреса ячеек в формуле СТРОГО СОВПАДАЮТ с таблицей! Если в задаче нужно проверить строку 2, то в формуле должно быть B2 (а не какой-то случайный диапазон).
        4. ЭКРАНИРОВАНИЕ: В массиве "expected" экранируй внутренние кавычки (например: "=ЕСЛИ(B2>=500; \\"Премия\\"; \\"Оклад\\")").
        5. РАЗНООБРАЗИЕ: Делай разные таблицы, не только про продажи (например, оценки учеников, учет товаров на складе и т.д.).`;
    ...
    const checkAnswer = () => {
        ...
        // Удаляем все пробелы для точной проверки
