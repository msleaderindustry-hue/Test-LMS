...
const ExcelTrainerLMS = ({ onBack }) => {
    ...
    const generateAIFormula = async (formulaName) => {
        ...
        // ЖЕСТКИЙ ПРОМПТ ДЛЯ ИИ: Добавлено правило креативности
        const prompt = `Ты профессиональный и очень креативный преподаватель Microsoft Excel. 
        ...
        Верни ТОЛЬКО чистый валидный JSON (без markdown) строго в таком формате:
        {
          ...
          "table": [
            ["Персонаж", "Урон", "Класс"],
            ["Эльф-маг", 450, "Дальний бой"],
            ["Орк-воин", 520, "Ближний бой"]
          ],
        ...
        4. ЭКРАНИРОВАНИЕ: В массиве "expected" экранируй внутренние кавычки (например: "=ЕСЛИ(B2>=500; \\"Огонь\\"; \\"Лед\\")").
        5. ТИПЫ ДАННЫХ: Формула в "expected" должна быть на 100% рабочей.
        6. УЛЬТРА-КРЕАТИВНОСТЬ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать скучные офисные темы (Сотрудники, Менеджеры, Продажи, Иванов, Петров, Отделы). Развивай воображение ученика! Используй темы: видеоигры (RPG, шутеры, Minecraft), космос и колонизация планет, киберпанк, магия, инвентарь супергероев, выживание в зомби-апокалипсисе. Каждая таблица должна быть захватывающей!`;
