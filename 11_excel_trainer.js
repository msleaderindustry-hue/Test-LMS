...
const ExcelTrainerLMS = ({ onBack }) => {
    ...
    const generateAIFormula = async (formulaName) => {
        ...
        // ЖЕСТКИЙ ПРОМПТ ДЛЯ ИИ: Добавлено правило для подробного и детского объяснения (def)
        ...
        Верни ТОЛЬКО чистый валидный JSON (без markdown) строго в таком формате:
        {
          ...
          "def": "Подробное, простое и живое объяснение работы функции для школьника с коротким примером из жизни. Минимум 2-3 предложения.",
          "taskDesc": "Напишите формулу, которая посчитает [ЧТО-ТО].",
          "table": [
            ["Товар", "Цена", "Категория"],
            ["Ноутбук", 45000, "Техника"],
            ["Стол", 5200, "Мебель"]
          ],
        ...
        2. ФОРМУЛИРОВКА ЗАДАЧИ: В поле "taskDesc" КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО упоминать ячейку для вывода результата (не пиши "для ячейки C5", "в ячейке B7" и т.д.). Пиши строго по шаблону: "Напишите формулу, которая посчитает..." И всё!
        ...
        4. ЭКРАНИРОВАНИЕ: В массиве "expected" экранируй внутренние кавычки (например: "=ЕСЛИ(B2>=500; \\"Дорого\\"; \\"Дешево\\")").
        5. ТИПЫ ДАННЫХ: Формула в "expected" должна быть на 100% рабочей.
        6. РЕАЛИСТИЧНОЕ РАЗНООБРАЗИЕ: Избегай заезженных офисных тем (Иванов, Петров, Менеджеры, Продажи). Используй разнообразные РЕАЛЬНЫЕ жизненные сценарии: школьные оценки, статистика спортивных матчей, меню ресторана, расходы на ремонт, учет автомобилей, погодные данные, бюджет на отпуск, склад магазина техники. БЕЗ фэнтези и магии! Только реальный мир.
        7. ОПРЕДЕЛЕНИЕ ("def"): Объясняй работу функции очень подробно, простым и живым языком, чтобы понял даже школьник. Обязательно приведи короткий пример из жизни. Никаких сухих энциклопедических фраз, пиши увлекательно!`;
