...
@keyframes pt-spin{to{transform:rotate(360deg)}}@keyframes pt-appear{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}@keyframes pt-caret{50%{opacity:0}}
@media(max-width:720px){.pt-shell{padding:25px;border-radius:24px}.pt-stats{gap:32px}.pt-stat{min-width:65px}.pt-stat strong{font-size:29px}.pt-stat.speed strong{font-size:45px}.pt-text{font-size:24px;max-height:138px}.pt-header-actions .pt-btn{padding:9px 13px}.pt-key{height:35px;font-size:11px}.pt-key.special{font-size:8px}}
@media(max-width:480px){.pt-shell{padding:22px 18px 18px;border-radius:22px}.pt-header{margin-bottom:28px;gap:8px}.pt-brand h2{font-size:26px}.pt-brand p{font-size:8px;letter-spacing:1.3px}.pt-header-actions{gap:4px}.pt-header-actions .pt-btn{padding:8px 11px;font-size:10px!important}.pt-header-actions>.pt-icon-btn{padding:7px}.pt-icon-btn{padding:7px}.pt-stats{justify-content:space-between;gap:10px;margin-bottom:27px}.pt-stat{min-width:0}.pt-stat-title{font-size:8px;gap:0;white-space:nowrap;margin-bottom:4px}.pt-stat strong{font-size:25px;letter-spacing:-1px}.pt-stat.speed strong{font-size:36px}.pt-stat small{font-size:8px;margin-left:2px}.pt-controls{margin-bottom:21px;gap:9px}.pt-segment button{padding:6px 13px;font-size:10px}.pt-mode-label{padding-left:11px;font-size:9px}.pt-ai{padding:14px}.pt-ai form{flex-wrap:wrap}.pt-topic{flex-basis:100%;font-size:16px!important}.pt-ai form .pt-btn{flex:1}.pt-practice-top{font-size:8px;letter-spacing:.3px;margin-bottom:12px}.pt-text{font-size:21px;line-height:1.85;max-height:167px;letter-spacing:-.5px}.pt-practice-bottom{font-size:9px;align-items:flex-start;gap:6px}.pt-start-hint{font-size:10px!important}.pt-feedback.error{flex-wrap:wrap}.pt-pause-hint{font-size:8px}.pt-keyboard-section{margin-top:25px}.pt-keyboard{gap:4px}.pt-key-row{gap:3px}.pt-key{height:30px;font-size:9px;border-radius:5px}.pt-key.special{font-size:6px}.pt-key.space{height:26px;max-width:145px;font-size:7px}.pt-key.home:after{bottom:4px;width:4px;left:calc(50% - 2px)}.pt-results{padding:14px 0 10px}.pt-results h3{font-size:25px}.pt-results-grid{gap:6px}.pt-result-stat{padding:8px 2px}.pt-result-stat strong{font-size:30px}.pt-result-stat span{font-size:9px}.pt-bottom{font-size:8px;gap:8px;margin-top:18px;padding-top:13px}.pt-bottom .pt-formula{display:none}}
...
function TypingTest({ onBack }) {
    ...
    return <div className="pt-root"><section className="pt-shell" aria-label="Тренажёр печати ProType">
        <header className="pt-header"><div className="pt-brand"><div><h2>Pro<span>Type</span></h2><p>Найди свой ритм</p></div></div><div className="pt-header-actions">{onBack && <button type="button" className="pt-icon-btn" onClick={onBack} title="Назад" aria-label="Назад"><Icon name="back" size={17}/></button>}<Button aria-expanded={showAI} onClick={() => { setShowAI(value => !value); setShowMenu(false); }}><Icon name="spark" size={15}/>Текст с ИИ</Button><div className="pt-menu-wrap" ref={menuRef}><button type="button" className="pt-icon-btn" aria-label="Настройки тренировки" aria-expanded={showMenu} title="Настройки тренировки" onClick={() => setShowMenu(value => !value)}><Icon name="more" size={20}/></button>{showMenu && <div className="pt-menu" aria-label="Настройки тренировки"><button type="button" disabled={generating} onClick={() => { setShowMenu(false); reset(true); }}><Icon name="refresh" size={15}/>Повторить текст</button><button type="button" disabled={generating} onClick={() => { setShowMenu(false); reset(false); }}><Icon name="play" size={15}/>Другой текст</button><button type="button" onClick={() => { setShowKeyboard(value => !value); setShowMenu(false); }}><Icon name="keyboard" size={15}/>{showKeyboard ? 'Скрыть клавиатуру' : 'Показать клавиатуру'}</button></div>}</div></div></header>
        ...
        <div className="pt-controls"><div className="pt-segment" aria-label="Язык текста">{[['en', 'English'], ['ru', 'Русский']].map(([lang, label]) => <button key={lang} type="button" className={game.lang === lang ? 'selected' : ''} aria-pressed={game.lang === lang} onClick={() => changeLanguage(lang)}>{label}</button>)}</div><span className="pt-mode-label">Свободный темп</span></div>
