...
const HotkeyTrainer = ({ onBack }) => {
    ...
    const [language, setLanguage] = useState("ru"); // ru, en, uz
    ...
    const generateAIHotkeys = async () => {
        ...
        1. НЕ ПРИДУМЫВАЙ комбинации. Используй только те горячие клавиши, которые реально существуют и задокументированы.
        2. Если для "${topic}" в принципе не существует 10 разных официальных комбинаций с Ctrl/Cmd — верни сколько есть (не меньше 5).
        3. Никакой отсебятины в описаниях.
        4. ВАЖНО: Описание действия (поле "desc") должно быть строго на ${targetLang} языке!
        5. Поле "key" — ТОЛЬКО ОДНА строчная английская буква или символ.
        6. Не повторяй одну и ту же комбинацию дважды.
        7. Верни ТОЛЬКО чистый валидный JSON-массив объектов. Без markdown, без пояснений.

        Формат строго такой:
        [
          {"desc": "Описание действия на ${targetLang}", "key": "c", "shift": false, "visual": "Ctrl + C"},
          {"desc": "Сохранить как", "key": "s", "shift": true, "visual": "Ctrl + Shift + S"}
        ]`;

        try {
            console.log(`🚀 Запрашиваем хоткеи у ИИ на ${targetLang}...`);
    ...
    if (!gameStarted && showTheory) {
        return (
            <motion.div
                ...
                style={{
                    width: '100%', maxWidth: '820px', display: 'flex', flexDirection: 'column',
                    gap: '22px', padding: '46px 34px', margin: '0 auto', position: 'relative'
                }}
            >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Button variant="muted" onClick={() => setShowTheory(false)}>Назад</Button>
                </div>
                ...
                <div style={{ 
                    ...
                    {activeHotkeys.map((hk, i) => (
                        <div key={i} style={{
                            padding: '12px 16px', background: 'var(--bg-body)', borderRadius: '12px',
                            border: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', gap: '6px'
                        }}>
                            <span style={{ fontSize: '16px', fontWeight: 800, color: '#8b5cf6' }}>{hk.visual}</span>
                            <span style={{ fontSize: '14px', color: 'var(--text-sec)', lineHeight: '1.4' }}>{hk.desc}</span>
                        </div>
                    ))}
                </div>

                <div style={{ display: 'flex', justifyContent: 'center', marginTop: '10px' }}>
                    <Button variant="orange" onClick={startGame} style={{ width: '100%', maxWidth: '300px', height: '52px', fontSize: '16px', borderRadius: '14px' }}>
                        🚀 Перейти к практике
                    </Button>
                </div>
            </motion.div>
    ...
    if (!gameStarted) {
        return (
            ...
            >
                ...
                {/* ПАНЕЛЬ ГЕНЕРАЦИИ И ВЫБОРА ЯЗЫКА */}
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-sec)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>
                            Своя база для другой программы
                        </div>
                        <div style={{ display: 'flex', gap: '4px' }}>
                            {['ru', 'en', 'uz'].map(lang => (
                                <button key={lang} onClick={() => setLanguage(lang)} style={{
                                    padding: '4px 8px', fontSize: '11px', fontWeight: 'bold', borderRadius: '6px', cursor: 'pointer',
                                    border: language === lang ? '1px solid #8b5cf6' : '1px solid var(--glass-border)',
                                    background: language === lang ? 'rgba(139,92,246,0.1)' : 'transparent',
                                    color: language === lang ? '#8b5cf6' : 'var(--text-sec)',
                    ...
                    <AnimatePresence>
                        {activeHotkeys !== HOTKEYS_DB && !isGenerating && (
                            ...
                            >
                                ✅ База «{topic}» успешно загружена ({language.toUpperCase()})
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>

                <div style={{ display: 'flex', gap: '14px', marginTop: '10px', width: '100%', maxWidth: '480px', justifyContent: 'center' }}>
                    <Button variant="muted" onClick={() => setShowTheory(true)} style={{ flex: 1, height: '52px', fontSize: '15px', borderRadius: '14px', border: '1px solid var(--glass-border)' }}>
                        📖 Теория
                    </Button>
                    <Button variant="orange" onClick={startGame} style={{ flex: 1, height: '52px', fontSize: '15px', borderRadius: '14px' }}>
                        🚀 Практика
                    </Button>
                </div>
                <div style={{ marginTop: '-4px' }}>
                    <Button variant="red" onClick={onBack} style={{ height: '42px', padding: '0 30px', fontSize: '14px', borderRadius: '14px', background: 'transparent', border: '1px solid #ef4444' }}>
                        Назад
                    </Button>
                </div>
            </motion.div>
